...
export function useProducts(
  ...
  async function fetchProducts(searchParams: Partial<ProductsSearchParams>, cartMode?: boolean) {
    loading.value = true;
    products.value = [];
    total.value = 0;
    pages.value = 1;
    ...
    try {
      const {
        ...
        range_facets = [],
        totalCount = 0,
      } = await searchProducts(searchParams, { withFacets, withImages, withZeroPrice }, Boolean(cartMode));
